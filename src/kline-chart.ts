...
export function computeMA(klines: readonly KlineBar[], period: number): number[] {
  ...
  for (let i = 0; i < klines.length; i++) {
    ...
    for (let j = i - period + 1; j <= i; j++) {
      const bar = klines[j]
      if (bar !== undefined) sum += bar.close
    }
    r.push(sum / period)
  }
  return r
...
function renderCandles(
  ...
  for (let row = CHART_ROWS - 1; row >= 0; row--) {
    ...
    for (let col = 0; col < bars.length; col++) {
      ...
      const nearMa = (v: number | undefined, sym: string) =>
        v !== undefined && Number.isFinite(v) && Math.abs(price - v) < halfRange ? sym : ""

      if (price <= bodyTop && price >= bodyBot) cells += `${c}█${ANSI.reset}`
      else if (price <= k.high && price >= k.low) cells += `${c}│${ANSI.reset}`
      else {
        const dot =
          nearMa(ma5[col], `${ANSI.brightWhite}·${ANSI.reset}`) ||
...
function renderVolume(
  ...
  const t =
    maxVol >= 100_000_000
      ? `${(maxVol / 100_000_000).toFixed(1)}亿`
      : maxVol >= 10_000
        ? `${(maxVol / 10_000).toFixed(0)}万`
        : `${maxVol}`
  result.push(
    fitLine(`${" ".repeat(gutter + 1)}${ANSI.brightBlack}成交量 最大 ${t}${ANSI.reset}`, width),
  )
  return result
}
