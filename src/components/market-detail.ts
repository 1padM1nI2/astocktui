...
import { renderKlineChart } from "../kline-chart"
...
export function renderMarketDetail(view: MarketDetailView): string[] {
  ...
  if (view.quote === undefined) {
    lines.push(
      fitLine(` ${ANSI.bold}${view.name}${ANSI.reset} ${ANSI.brightBlack}${view.code}${ANSI.reset}`, w),
      "",
  ...
  // 主价格行：现价 + 涨跌幅 + 涨跌额
  const chg = quote.prevClose !== undefined ? quote.price - quote.prevClose : undefined
  ...
  const openColor =
    tk !== undefined && quote.prevClose !== undefined
      ? tk.open > quote.prevClose
        ? ANSI.red
        : tk.open < quote.prevClose
          ? ANSI.green
          : ""
      : ""
  const amplitude =
    quote.high !== undefined && quote.low !== undefined && quote.prevClose
      ? ((quote.high - quote.low) / quote.prevClose) * 100
      : undefined
  const volume = quote.volume ?? tk?.volume
  const pairs: readonly (readonly [string, string])[] = [
    [`今开 ${fmtPrice(tk?.open, openColor)}`, `最高 ${fmtPrice(quote.high, ANSI.red)}`],
    [`最低 ${fmtPrice(quote.low, ANSI.green)}`, `昨收 ${fmtPrice(quote.prevClose, ANSI.brightBlack)}`],
    [
  ...
  if (view.klines !== undefined && view.klines.length >= 2) {
    lines.push("", fitLine(` ${ANSI.brightBlack}日K线图（近${view.klines.length}日）${ANSI.reset}`, w))
