...
export class MarketSelectionController {
  ...
  #headerRows = DETAIL_HEADER_ROWS

  /** 列表正文在滚动视图中的起始行（标题/表头等占位），由渲染方按实际布局设置 */
  setHeaderRows(rows: number): void {
    this.#headerRows = Math.max(0, rows | 0)
  }
  ...
  #handleScrollInput(data: string): boolean {
    ...
    this.scroll.ensureVisible(this.#selectedIndex + this.#headerRows)
  }
}
