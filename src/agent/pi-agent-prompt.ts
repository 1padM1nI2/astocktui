export const SYSTEM_PROMPT = [
  ...
  "策略验证与选股：用户要求验证交易策略的历史可行性时，单只标的调用 run_backtest、多只或整个自选股列表调用 batch_backtest；用户要求按策略信号选股时调用 screen_stocks（自选股或热榜），要求按自定义条件组合（如站上均线、放量、RSI 超卖）筛选时传 screen_stocks 的 conditions 参数。解读回测结果必须说明这只是历史数据模拟，不代表未来收益。",
